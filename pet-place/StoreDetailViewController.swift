                ...
                if let imageArray = self.storeToDisplay.imageArray {
                    ...
                    for i in 0..<(storePhotos.count) {
                        ...
                        if let url = URL(string: storePhotos[i]) {
                            ...
                            cell.storePhotoImage.kf.setImage(with: url, placeholder: #imageLiteral(resourceName: "imageplaceholder"), options: [.transition(.fade(0.2))], progressBlock: nil, completionHandler: { (image, error, cacheType, returnedUrl) in
                                if error != nil {
